...
function tile_window_workspace(meta_window) {
    ...
    if (!workspace)
        return;
    tiling.tile_workspace_windows(workspace, meta_window, null, false);
}
...
export default class MosaicExtension extends Extension {
    ...
    destroyed_handler(_, win) {
        ...
        if (monitor === global.display.get_primary_monitor()) {
            tiling.tile_workspace_windows(windowing.get_workspace(), global.display.get_focus_window(), null, true);
            let workspace = window.get_workspace();
        ...
        if (windowing.is_related(window)) {
            ...
            if (mode === Meta.SizeChange.FULLSCREEN || mode === Meta.SizeChange.MAXIMIZE) { // If the window was maximized
                ...
                if ((window.maximized_horizontally === false ||
                    ...
                    let maxInfo = maximized_windows[id];
    ...
    enable() {
        ...
        tile_timeout = setInterval(this.tile_all_workspaces.bind(this), 60000 * 5); // Tile all windows every 5 minutes (in case the machine/display goes to sleep)
    }
    disable() {
        console.log("[MOSAIC]: Disabling Mosaic layout manager.");
        // Disconnect all events
        if (tile_timeout) {
            clearInterval(tile_timeout);
